...
1. Serach through the ./docs-provider files, and check if the relevant information for the docs is there. If there, then check if the information is enough for the task at hand. If so, then use that particular .md file, for contexnt, then you don't have to do anything else. If not, then go to the next step.  
